...
def gen_APPLY():
    macros = []
    macros.append(
        "APPLY(macro, OP, ...) CAT_BASE(APPLY, IS_EMPTY(__VA_ARGS__))(macro, FWD_TOKEN(OP), __VA_ARGS__)"
    )
    macros.append("APPLY1(macro, OP, ...)")
    macros.append(
        "APPLY0(macro, OP, ...) CAT_BASE(APPLY_, PP_NARG(__VA_ARGS__))(macro, OP, __VA_ARGS__)"
    )

    macros.append(
        "APPLY_TOO(macro, OP, ...) CAT_BASE(APPLY_TOO, IS_EMPTY(__VA_ARGS__))(macro, FWD_TOKEN(OP), __VA_ARGS__)"
    )
    macros.append("APPLY_TOO1(macro, OP, ...)")
    macros.append(
        "APPLY_TOO0(macro, OP, ...) CAT_BASE(APPLY_, PP_NARG(__VA_ARGS__))(macro, OP, __VA_ARGS__)"
    )
