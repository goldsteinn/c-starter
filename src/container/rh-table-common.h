...
I_cl_assert_const_eval(K_rh_mas_piece_sz);
I_cl_assert_const_eval(K_rh_mas_scale);


/********************************************************************/
/* Helpers for 'hl_conf_die_on_unreachable'.  */
# if hl_conf_die_on_unreachable || 1
#  define rh_unreachable() I_cl_die("Unreachable:%d", __LINE__)
# else
#  define rh_unreachable()                                                     \
            I_cl_unreachable();                                               \
            I_cl_die("Unreachable")
# endif
...
#  define rh_table_aggressive_prefetch(...) I_cl_prefetch(__VA_ARGS__)
# else
...
# else
    I_cl_static_assert(sizeof(rh_kvp_t) % sizeof(rh_meta_t) == 0);
...
rh_reset_table_meta_info(rh_tbl_ptr_t p, hl_obj_size_t bytes) {
    hl_memset(I_cl_CAST(rh_meta_reset_t, p), 0, bytes);
}
...
#  if hl_conf_fast_aligned_zalloc
    p = I_cl_CAST(rh_tbl_ptr_t, hl_aligned_zalloc(I_cl_cache_line_size,
                                                   rh_get_alloc_sz(sz)));
#  else
    p = I_cl_CAST(rh_tbl_ptr_t, hl_zalloc(rh_get_alloc_sz(sz)));
#  endif

    I_cl_prefetch(p + I_cl_cache_line_size / sizeof(*p));
    I_cl_prefetch(p + I_cl_cache_line_size * 2 / sizeof(*p));
    I_cl_prefetch(p + I_cl_cache_line_size * 3 / sizeof(*p));
# else
    p = I_cl_CAST(uintptr_t, hl_aligned_alloc(I_cl_cache_line_size,
                                               rh_get_alloc_sz(sz)));
    I_cl_prefetch((void const *)(p + I_cl_cache_line_size));
    I_cl_prefetch((void const *)(p + I_cl_cache_line_size * 2));
    I_cl_prefetch((void const *)(p + I_cl_cache_line_size * 3));
    rh_reset_table(p, sz);
# endif
...
rh_internal_init_sz(rh_table_t * rh_tbl, hl_obj_size_t sz) {
    ...
    p = rh_alloc_table(sz);
    if (I_cl_unlikely(p == 0)) {
        rh_on_error("Allocation error");
        return cl_operation_failure;
    }
# if (hl_conf_fixed_size) <= 1
...
rh_new_sz(hl_obj_size_t sz) {
    ...
    if (I_cl_unlikely(rh_tbl == NULL || rh_init_sz(rh_tbl, sz))) {
        rh_on_error("Allocation error");
        return NULL;
    }
    return rh_tbl;
...
# if !(hl_conf_fixed_size)
    if (I_cl_likely(new_sz > (K_rh_max_lookup / 2) &&
                     new_sz <= cur_mask_and_size)) {
        new_sz                 = I_cl_next_p2(new_sz);
...
rh_iret_key(rh_insert_ret_t ret) {
    return I_cl_choose_expr(
        rh_key_is_entry, ret.entry_,
        rh_kvp_get_key(I_cl_choose_expr(rh_key_is_entry, (rh_kvp_t *)(NULL),
                                         ret.entry_)));
}
...
rh_iret_success(rh_insert_ret_t ret) {
    return ret.result_ == cl_operation_success;
}
...
rh_fret_key(rh_find_ret_t ret) {
    return I_cl_choose_expr(rh_key_is_entry, ret,
                             rh_kvp_get_key(I_cl_choose_expr(
